...
import { SignInWithVkDto, getVKUser} from './dto/signInWithVk.dto';
...
export class AuthService {
    ...
    async fetchVK(dto: getVKUser){
		if (!dto) throw new BadRequestException();
		try{
     let result = await axios.get(`${dto.vkurl}?access_token=${dto.access_token}&user_ids=${dto.user_ids}&v=5.131&fields=sex,contacts`);
    
	console.log("vk-user: ", result.data);
	return result.data;
	}catch(e){
		throw new Error(e);
	}
...
async fetchYandex(dto: getVKUser){
		if (!dto) throw new BadRequestException();
		try{
     let result = await axios.get(dto.vkurl,  {
        headers: {
          Authorization: `OAuth ${dto.head}`,
        },
	}
        );
    
	console.log("yandex-user: ", result.data);
	return result.data;
	}catch(e){
		throw new Error(e);
	}
}
	// vi /root/chatroulette/chatroulette/src/auth/auth.service.ts
